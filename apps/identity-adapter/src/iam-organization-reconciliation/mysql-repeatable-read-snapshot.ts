...
export const ORGANIZATION_RECONCILIATION_MYSQL_STATEMENTS = Object.freeze({
  ...
  "identity-subject-universe-page/v3":
    "SELECT legacy_user_id, status, source FROM identity_users WHERE legacy_user_id IS NOT NULL AND source = 'legacy-shadow' AND status = 'active' AND legacy_user_id > ? ORDER BY legacy_user_id ASC LIMIT ?",
